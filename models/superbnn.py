...
def superbnn_cifar10(sub_path=None):
    # (channels, num_blocks, ks, groups1, groups2, stride)
    # This is a scaled-down CFG suitable for CIFAR-10. 
    # It has fewer stages and smaller channel counts compared to the ImageNet one.
    # Strides are adjusted to take 32x32 input down to a reasonable feature map size (e.g., 4x4 or 2x2).
    # Current strides: 32 -> 16 (s2) -> 16 (s1) -> 8 (s2) -> 4 (s2) -> 2 (s2)
    # This might still be a bit deep, but let's start with the paper's block structure.
    # You might need to experiment with this configuration later for optimal CIFAR-10 performance.
    cfg = [
        [[16, 24, 32], [1], [3], [1], [1], 2],  # Output: 16x16
        [[32, 48, 64], [1, 2], [3], [1], [1], 1],  # Output: 16x16
        [[64, 96, 128], [1, 2], [3, 5], [1, 2], [1], 2],  # Output: 8x8
        [[128, 196, 256], [1, 2], [3, 5], [2, 4], [1], 2],  # Output: 4x4
        # Optional: Add another stage if needed, or stop here for smaller models
        # [[256, 384, 512], [1, 2], [3, 5], [4, 8], [1], 2] # Output: 2x2
    ]
    # For CIFAR-10, the number of classes is 10 and image size is 32x32
